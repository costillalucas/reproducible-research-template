...
This is the original worked example that shipped with the template,
frozen here for reference — see `examples/pi_estimator/README.md` for how
it relates to the live project. It exists to prove the mechanism end to
end, not to say anything interesting about pi: every number below was read
from `examples/pi_estimator/numbers.json`, written *only* by
`examples/pi_estimator/compute_numbers.py`. Re-run that script and
`examples/pi_estimator/checks.py` any time to regenerate both files; the
shared `scripts/check_provenance.py` gate can't be pointed at this copy
directly (its registry/structure paths are hardcoded to the repo root),
so treat the numbers below as illustrative, not gate-checked in place.
...
`examples/pi_estimator/checks.py` [src:suite_coverage] — output pasted
...
- Any estimator other than the two implemented in
  `examples/pi_estimator/pi_estimator.py`.
